...
export async function getForm({ formId }) {
    try {
        ...
        if (formId) {
            conditions.formId = formId;
        }

        // console.error(`[DB getForm] Params (params):`, conditions);
        const result = await prisma.form.findFirst({
            where: {
                ...conditions
            }
        });
        // console.error(`[DB getForm] Return (result):`, result);
        return result;
    } catch (error) {
        console.error(`[DB getForms] Error (error):`, error);
    }
...
export async function updateForm({ formId, text, choices, maxchoices, status, frozen }) {
    try {
